    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Browse all available courses across colleges and departments
        </p>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {courses.filter((c) => c.courseType === 'GST').length}
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.slice(0, 60).map((course) => {
          const typeInfo = COURSE_TYPES.find(
            (t) => t.value === course.courseType
          );
          return (
            <Card
              key={course.id}
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {typeInfo && (
                      <Badge className={cn('text-[9px] py-0', typeInfo.color)}>
                        {typeInfo.label}
                      </Badge>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2 text-[10px] text-muted-foreground">
                  ...
                  {course.department && (
                    <span className="truncate">· {course.department.code}</span>
                  )}
                </div>
              </CardContent>
            </Card>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    className={cn(
                      'text-[10px]',
                      COURSE_TYPES.find(
                        (t) => t.value === selectedCourse.courseType
                      )?.color
                    )}
                  >
                    {
                  ...
                  <Badge variant="outline" className="text-[10px]">
                    {selectedCourse.creditUnits} Credit Units
                  </Badge>
                </div>
              ...
              <div className="space-y-4 py-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-3 rounded-lg bg-muted/40">
                    ...
                    <p className="font-semibold text-sm">
                      {selectedCourse.level.replace('LEVEL_', '')} Level
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/40">
                    <p className="text-xs text-muted-foreground">Semester</p>
                    <p className="font-semibold text-sm">
                      {selectedCourse.semester === 'FIRST'
                        ? 'First Semester'
                        : 'Second Semester'}
                    </p>
                  </div>
                ...
                {selectedCourse.offerings &&
                  selectedCourse.offerings.length > 0 && (
                    <div>
                      ...
                      {selectedCourse.offerings[0].timetable ? (
