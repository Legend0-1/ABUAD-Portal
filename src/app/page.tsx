...
export default function Home() {
  ...
  async function checkAuth() {
    try {
      ...
      // Access token likely expired (15 min lifespan) - try to silently refresh it
      // using the longer-lived refresh token before forcing a re-login.
      try {
  ...
  const getDashboardScope = () => {
    switch (user.role) {
      ...
      case 'COLLEGE_OFFICER':
        return 'college_officer';
      case 'DEPARTMENT_COORDINATOR':
        return 'coordinator';
      case 'ACADEMIC_ADVISER':
        return 'adviser';
      default:
        return 'super_admin';
    }
