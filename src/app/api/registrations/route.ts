...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user.role !== 'STUDENT') {
      return NextResponse.json(
        { error: 'Only students can create registrations' },
        { status: 403 }
      );
    }
    ...
    // Find active session & semester first, since Payment has no "session" relation
    // (only a scalar sessionId), so we need the session id before filtering payments.
    ...
    const student = await db.student.findUnique({
      where: { userId: user.id },
      include: {
        payments: { where: { sessionId: activeSession.id } },
        registrations: {
    ...
    if (unpaidFees.length > 0) {
      return NextResponse.json(
        {
          error: `Payment verification required. Outstanding fees: ${unpaidFees
            .map((f) => f.feeType.replace('_', ' '))
            .join(', ')}`,
    ...
    const existingReg = student.registrations.find(
      (r) => r.sessionId === activeSession.id
    );
    if (existingReg && existingReg.status === 'APPROVED') {
      return NextResponse.json(
        { error: 'Registration already approved and locked' },
        { status: 400 }
      );
    }
    ...
    const maxUnits =
      student.level === 'LEVEL_500' ? MAX_CREDIT_UNITS_500 : MAX_CREDIT_UNITS;
    ...
    for (const course of courses) {
      if (course.level !== student.level && course.level !== 'LEVEL_100') {
        ...
        if (course.courseType !== 'GST') {
          return NextResponse.json(
            {
              error: `${course.code} is for ${course.level.replace(
                'LEVEL_',
                ''
              )} level students only`,
            },
      ...
      if (course.semester !== activeSemester.name) {
        return NextResponse.json(
          {
            error: `${course.code} is not offered in ${activeSemester.name
              .replace('FIRST', 'First')
              .replace('SECOND', 'Second')} semester`,
          },
          { status: 400 }
        );
      }
    ...
    const coreUnits = courses
      .filter((c) => c.courseType === 'CORE')
      .reduce((s, c) => s + c.creditUnits, 0);
    const electiveUnits = courses
      .filter((c) => c.courseType === 'ELECTIVE')
      .reduce((s, c) => s + c.creditUnits, 0);
    const gstUnits = courses
      .filter((c) => c.courseType === 'GST')
      .reduce((s, c) => s + c.creditUnits, 0);
    ...
    await logAction(
      ...
      'REGISTRATION',
      `${
        action === 'submit' ? 'Submitted' : 'Saved draft'
      } course registration with ${
        courseIds.length
      } courses (${totalUnits} units)`,
      req,
